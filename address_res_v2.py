...
- Make a difference when the Function=1 or not. [PENDING]
'''
...
def find_extgnb_partner_function(site:str, content:str):
    ...
    if match:
        ...
        partner_functions_1 = re.findall(r'ExtGNBDUPartnerFunction=1\s+\w+\s+\s+(\d+)', mo_text)# Added to capture site ID for ExtGNBDUPartnerFunction=1
        partner_functions = re.findall(r'ExtGNBDUPartnerFunction=\s*\d+\s+\w+\s+(\d+)', mo_text)

        if partner_functions_1:
            output += f"ExtGNBDUPartnerFunction=1 identified: {', '.join(partner_functions_1)}\n\n"
        
        if partner_functions:
            output += f"All ExtGNBDUPartnerFunction values found: {', '.join(partner_functions)}\n\n"
            ...
            if match3:
                ...
                for i in partner_functions:
                    ...
                    if term_points_gnb:
                        ...
                        output += f"-> No TermPointToGNB values found in the MOs {i}<-\n"
            ...
            if match2:
                for i in partner_functions:
                    # Search for the pattern in each line of match2.group(1)
                    for line in match2.group(1).split('\n'):
                        # Check if both 'ExternalGNodeBFunction' and the partner function item appear in the line
                        if 'Function=' in line and i in line:
                            # Process the line here
                            output += f"TermPoint: {line.strip()}\n"
...
def read_site_names(file_path):
    """Read site names from a text file."""
    with open(file_path, 'r') as file:
        site_names = [line.strip() for line in file if line.strip()]
    if not site_names:
        # Do something if site_names is empty
        print("Site names list is empty!")
    return site_names
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
        ...
        folder_path = '/home/shared/common/Overnight_Work/Rehomes/Prechecks'  # Default value
